...
class Timer(object):
    """
    Timer that gives you time elapsed in a context
    """
    def __enter__(self):
    ...
    def interval(self) -> float:
        """
        Returns the elapsed time from the start to end of context
        """
...
def parse_args():
    """
    Parses and returns a namespace argument giving the arguments of the program
    """
    ...
    parser.add_argument("-p", "--prompt", dest="prompt_id", type=int, metavar='<int>', required=True,
...
def pad_sequences(vectorized_seqs, maxlen=None):
    if maxlen is None:
    ...
    # 2-d mask to turn features on or off
    ...
    # Propagate the permutation changes
    seq_lengths, perm_idx = seq_lengths.sort(0, descending=True)
    seq_tensor = seq_tensor[perm_idx]
    tensor_masque = tensor_masque[perm_idx]
...
def one_hot(np_y: np.array, mapping: Dict[float, int]) -> Variable:
    """

    :param np_y: A list of class values in float format
    :param mapping: Mapping of floats to integer indicies
        len(mapping) == max(mapping.values())
    :return: Tensorflow variable with the encoding
    """
    ...
    return Variable(zeros, requires_grad=False).float()
...
def main(args) -> None:
    ...
    if args.loss == 'mse':
        ...
        loss = F.l1_loss
        metric = F.mse_loss
    # Create labels statically because the training set isn't that good
    ...
    for ii in range(args.epochs):
        with Timer() as tr_time:
            calc_loss = train(model, optimizer, train_x_tuple, train_y, loss)
        total_train_time += tr_time.interval

        with Timer() as evl_time:
            # Evaluate
            pass
        total_eval_time += evl_time.interval
